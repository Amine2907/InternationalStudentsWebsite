...
const Phones = () => {
  ...
  const [activeButton, setActiveButton] = useState("Phones"); // Set activeButton to "Phones"
  ...
  useEffect(() => {
    const getProducts = async () => {
      ...
      if (componentMounted) {
        ...
        setFilter(await response.data.filter((item) => item.category === "Phones"));
        setLoading(false);
      }
      ...
      <>
        <div className="buttons text-center py-5">
          ...
          <button
            className={
              activeButton === "Phones"
                ? "btn btn-outline-dark btn-sm m-2 active" // Update class condition
                : "btn btn-outline-dark btn-sm m-2"
            }
